...
export default function SessionHistoryPage({
  ...
  return (
    <div>
      <Link
        href="/dashboard"
        className="mb-4 inline-flex items-center gap-1.5 text-sm text-text-secondary hover:text-teal"
      >
      ...
      {records.length === 0 ? (
        ...
        <>
          ...
          <div className="space-y-2 md:hidden">
            {records.map((r) => (
              <div key={r.id} className="rounded-lg border border-teal/10 bg-cream p-3">
                <div className="flex items-center justify-between gap-2">
                  <p className="min-w-0 truncate font-medium text-teal">
                ...
                {r.flagged ? (
                  <button
                    ...
                    className="mt-2 min-h-[36px] text-xs text-turquoise hover:underline"
                  >
                    Remove flag
                  </button>
                ) : (
          ...
          {/* Desktop: table */}
          <div className="hidden overflow-hidden rounded-lg border border-teal/10 md:block">
            <table className="w-full">
              <thead>
                <tr className="border-b border-teal/10 bg-cream text-left text-xs text-text-secondary">
              ...
              <tbody>
                {records.map((r) => (
                  <tr key={r.id} className="border-b border-teal/10 text-sm">
