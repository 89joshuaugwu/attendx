...
export function exportSessionRecordsPdf(opts: {
  ...
  doc.setFontSize(16);
  doc.text("AttendX — Attendance Report", 14, 18);
  ...
  autoTable(doc, {
    ...
    headStyles: { fillColor: [25, 181, 165] },
    styles: { fontSize: 9 },
  });
...
export function exportAnalyticsPdf(opts: {
  ...
  doc.setFontSize(16);
  doc.text("AttendX — Attendance Analytics", 14, 18);
  ...
  autoTable(doc, {
    ...
    headStyles: { fillColor: [217, 79, 79] },
    styles: { fontSize: 9 },
  });

  doc.save("attendx-analytics-report.pdf");
}
