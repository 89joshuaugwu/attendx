...
async function send(to: string, subject: string, html: string) {
  ...
  await transport.sendMail({
    from: `"AttendX" <${process.env.GMAIL_SMTP_USER}>`,
    to,
    subject,
    html,
  });
...
export async function sendSessionEndedEmail(
  ...
  const html = renderEmail({
    ...
    bodyHtml: `
      ...
      <p style="margin:0; color:#5c7d78; font-size:13px;">
...
export async function sendDuplicateDeviceAlert(
  ...
  const html = renderEmail({
    ...
    bodyHtml: `
      ...
      <p style="margin:16px 0 0; padding:12px 14px; background-color:#FDF3E4; border-left:3px solid #B76E00; border-radius:6px; font-size:13px; color:#7A4A00;">
...
export async function sendWeeklySummaryEmail(
  ...
  const html = renderEmail({
    ...
    heading: "Your weekly AttendX summary",
    bodyHtml: `
      <p style="margin:0 0 4px;">Hi ${lecturer.name.split(" ")[0]},</p>
  ...
  await send(lecturer.email, "Your weekly AttendX summary", html);
}
