...
function RotationCountdown({ qrTokenUpdatedAt }: { qrTokenUpdatedAt: number }) {
  ...
  return (
    <div className="flex w-full max-w-[340px] items-center gap-2">
      <div className="h-1.5 flex-1 overflow-hidden rounded-full bg-teal/15">
        <div
          className="h-full rounded-full bg-turquoise transition-[width] duration-500 ease-linear"
...
export function QRDisplay({
  ...
  return (
    <div className="flex flex-col items-center gap-3">
      <div className="relative flex items-center justify-center rounded-2xl border-2 border-turquoise bg-white p-3 shadow-lg">
        <QRCodeSVG
          value={value}
          size={280}
          level="M"
      ...
      <button
        onClick={copyLink}
        className="inline-flex min-h-[44px] items-center gap-1.5 rounded-lg border border-teal/12 px-3.5 text-xs text-text-secondary hover:bg-teal/5 hover:text-teal"
      >
